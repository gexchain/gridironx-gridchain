...
func (suite *KeeperTestSuite) TestSendToWasmEventHandler_Handle() {
	contractAccAddr, err := sdk.AccAddressFromBech32("fury1fnkz39vpxmukf6mp78essh8g0hrzp3gylyd2u8")
	suite.Require().NoError(err)
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				wasmAddrStr := suite.wasmContract.String()
				input, err := getSendToWasmEventData(wasmAddrStr, "fury111", big.NewInt(1))
				suite.Require().NoError(err)
				data = input
			},
			func() {
...
func (suite *KeeperTestSuite) TestKeeper_CallToEvm() {
	...
	testCases := []struct {
		...
		},
		{
			...
			nil,
			fmt.Sprintf(evmReturnPrefix, "fury1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqpxuz0nc") + fmt.Sprintf(callDataFormat, contract, "init-to-call-evm"),
		},
		{
...
func (suite *KeeperTestSuite) TestCallToWasmEventHandler_Handle() {
	tempAddr, err := sdk.AccAddressFromBech32("fury1eutyuqqase3eyvwe92caw8dcx5ly8s544q3hmq")
	suite.Require().NoError(err)
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				calldata := "{\"transfer1\":{\"amount\":\"100\",\"recipient\":\"fury1eutyuqqase3eyvwe92caw8dcx5ly8s544q3hmq\"}}"
				value := sdk.NewInt(0)
				data, err = getCallToWasmEventData(wasmContractAddr, value.BigInt(), hex.EncodeToString([]byte(calldata)))
				require.NoError(suite.T(), err)
			},
		...
		},
		{
			...
			func() {
				calldata := "{\"transfer\":{\"amount\":\"100\",\"recipient\":\"fury1eutyuqqase3eyvwe92caw8dcx5ly8s544q3hmq\"},\"transfer\":{\"amount\":\"100\",\"recipient\":\"fury1eutyuqqase3eyvwe92caw8dcx5ly8s544q3hmq\"}}"
				value := sdk.NewInt(0)
				data, err = getCallToWasmEventData(wasmContractAddr, value.BigInt(), hex.EncodeToString([]byte(calldata)))
				require.NoError(suite.T(), err)
			},
		...
		},
		{
			...
			func() {
				calldata := "{\"transfer_from\":{\"amount\":\"100\",\"recipient\":\"fury1eutyuqqase3eyvwe92caw8dcx5ly8s544q3hmq\"}}"
				value := sdk.NewInt(0)
				data, err = getCallToWasmEventData(wasmContractAddr, value.BigInt(), hex.EncodeToString([]byte(calldata)))
				require.NoError(suite.T(), err)
			},
