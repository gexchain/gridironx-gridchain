...
func (suite *KeeperTestSuite) TestKeeper_CallToWasm() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				calldata = "{\"transfer\":{\"amount\":\"100\",\"recipient\":\"did:fury:gridex1eutyuqqase3eyvwe92caw8dcx5ly8s544q3hmq\"}}"
			},
			func() {
...
func (suite *KeeperTestSuite) TestMsgServer_CallToEvmEvent() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				caller = "did:fury:gridex1231bdjasd1"
				evmInput, err = getCallByWasmInput(suite.evmABI, caller, callData)
				suite.Require().NoError(err)
			},
			func() {
