...
func TestConvertAccessConfig(t *testing.T) {
	furyex1, eth1 := generateAddress(1)
	...
	testcase := []struct {
		...
		{
			name:   "only 1 exaddr",
			config: AccessConfig{Permission: AccessTypeOnlyAddress, Address: strings.Join([]string{furyex1.String()}, ",")},
			expect: AccessConfig{Permission: AccessTypeOnlyAddress, Address: strings.Join([]string{eth1.String()}, ",")},
			isErr:  false,
		},
		{
			name:   "only 2 exaddr",
			config: AccessConfig{Permission: AccessTypeOnlyAddress, Address: strings.Join([]string{furyex1.String(), ex2.String()}, ",")},
			expect: AccessConfig{Permission: AccessTypeOnlyAddress, Address: strings.Join([]string{eth1.String(), eth2.String()}, ",")},
			isErr:  false,
		},
		{
