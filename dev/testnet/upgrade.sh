...
function get_latest_height() {
  ...
  height=`gridchaincli status --node http://${IP}:${port} | jq .sync_info.latest_block_height | awk '{ gsub(/"/,""); print $0 }'`
  echo $height
}
...
function add_val() {
  index=$1
  node_name=val${index}
  seed_addr=$(gridchaind tendermint show-node-id --home cache/node0/gridchaind)@${IP}:${seedp2pport}
  echo "add val >>> "$node_name

  gridchaind_opts="--p2p.allow_duplicate_ip  --p2p.pex=false  --p2p.addr_book_strict=false  --consensus.timeout_commit 600ms    --upload-delta=false  --elapsed DeliverTxs=0,Round=1,CommitRound=1,Produce=1  --consensus-role=v${index}  --p2p.seeds ${seed_addr} "

  start_node $index $node_name $gridchaind_opts
}
...
function add_seed() {
  ...
  gridchaind_opts="--p2p.seed_mode=true  --p2p.allow_duplicate_ip  --p2p.pex=false  --p2p.addr_book_strict=false  --consensus.timeout_commit 600ms  --upload-delta=false  --elapsed DeliverTxs=0,Round=1,CommitRound=1,Produce=1  --consensus-role=v$index "
...
function add_rpc() {
  ...
  gridchaind_opts="--p2p.seeds ${seed_addr} "
  start_node $index $node_name $gridchaind_opts
}
...
function case_1() {
  ...
  #STEP kill rpc
  killbyname_gracefully "cache/node4/gridchaind"
  sleep 2

  #STEP BUILD version2
  build_gridchain $version2
  ...
  #STEP kill 25% v
  killbyname_gracefully "cache/node3/gridchaind"
  sleep 3
...
function case_2() {
  ...
  #STEP upgrade 100% v
  killbyname_gracefully "cache/node2/gridchaind"
  sleep 3
  add_val 2

  killbyname_gracefully "cache/node1/gridchaind"
  sleep 3
  add_val 1

  killbyname_gracefully "cache/node0/gridchaind"
  sleep 3
  add_seed
  ...
  #STEP kill rpc
  killbyname_gracefully "cache/node4/gridchaind"
  killbyname_gracefully "cache/node5/gridchaind"
  sleep 3
...
### send two params , the first is the old version of gridchain, the second is the newer version.
exc_version1=$1
exc_version2=$2
